...
from GAT import ThreePartGAT, calculate_metrics
...
def train_model(model: ThreePartGAT,
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(train_ids), batch_size):
            ...
            try:
                ...
                # 添加批次大小
                edge_index_dict['batch_size'] = batch_data.get('batch_size', 1)
                # 处理边属性
                edge_attr_dict = None
        ...
        if avg_val_loss < best_val_loss:
            ...
            model_improved = True
            # 保存基于损失的最佳模型
            torch.save(model.state_dict(), "models/best_gat_model_loss.pt")
            print("保存基于损失的最佳模型")
        ...
        if len(all_outputs) > 0 and len(all_labels) > 0 and metrics['bullying']['f1'] > best_val_f1:
            ...
            model_improved = True
            # 保存基于F1的最佳模型
            torch.save(model.state_dict(), "models/best_gat_model_f1.pt")
            print("保存基于F1分数的最佳模型")
