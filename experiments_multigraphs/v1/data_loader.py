...
class DataLoader:
    ...
    def load_subgraph(self, media_session_id: str) -> HeteroData:
        ...
        with self.driver.session() as session:
            ...
            offensive_mention_edges = []
            non_offensive_mention_edges = []
            ...
            for rel in internal_relationships:
                ...
                elif rel_type == 'OFFENSIVE_MENTION':
                    if start_id in comment_id_to_idx and end_id in comment_id_to_idx:  # 评论间攻击性提及
                        offensive_mention_edges.append([start_id, end_id])

                elif rel_type == 'NON_OFFENSIVE_MENTION':
                    if start_id in comment_id_to_idx and end_id in comment_id_to_idx:  # 评论间非攻击性提及
                        non_offensive_mention_edges.append([start_id, end_id])
            ...
            # 处理评论间直接边
            # 攻击性提及边
            offensive_mention_edge_index = self._process_edge_index(
                offensive_mention_edges,
                {'comment': data['comment'].x.shape[0]},
                comment_id_to_idx,
                dst_id_to_idx=comment_id_to_idx
            )
            data['comment', 'OFFENSIVE_MENTION', 'comment'].edge_index = offensive_mention_edge_index

            # 非攻击性提及边
            non_offensive_mention_edge_index = self._process_edge_index(
                non_offensive_mention_edges,
                {'comment': data['comment'].x.shape[0]},
                comment_id_to_idx,
                dst_id_to_idx=comment_id_to_idx
            )
            data['comment', 'NON_OFFENSIVE_MENTION', 'comment'].edge_index = non_offensive_mention_edge_index
        ...
        for e in edge_data:
            if e[0] is not None and e[1] is not None:
                ...
                if dst_id_to_idx is not None and dst_id in dst_id_to_idx:
                    ...
                    # 对于没有映射的节点，使用简单的哈希映射
                    # 如果node_counts只有一个键，说明源节点和目标节点是同一类型
                    if len(node_counts) == 1:
                        dst_idx = hash(dst_id) % node_counts[list(node_counts.keys())[0]]
                    else:
                        dst_idx = hash(dst_id) % node_counts[list(node_counts.keys())[1]]

                # 验证索引是否有效
                if len(node_counts) == 1:
                    # 如果只有一个节点类型，源节点和目标节点使用相同的计数
                    is_valid = (src_idx < node_counts[list(node_counts.keys())[0]] and
                               dst_idx < node_counts[list(node_counts.keys())[0]])
                else:
                    # 如果有两个不同的节点类型
                    is_valid = (src_idx < node_counts[list(node_counts.keys())[0]] and
