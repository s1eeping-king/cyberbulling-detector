...
class CyberbullyingKnowledgeGraph:
    ...
    def build(self):
        ...
        try:
            ...
            num_edges = self.G.number_of_edges()  # 使用实际的边数
            avg_out_degree = sum(dict(self.G.out_degree()).values()) / num_nodes
            avg_in_degree = sum(dict(self.G.in_degree()).values()) / num_nodes
            
            logger.info(f"\nDegree Statistics:")
            logger.info(f"- Total unique edges: {num_edges}")
            logger.info(f"- Average out-degree: {avg_out_degree:.2f}")
            logger.info(f"- Average in-degree: {avg_in_degree:.2f}")
    ...
    def save_graph(self, path: str = 'knowledge_graph/knowledge_graph.pkl'):
        """保存知识图谱到文件"""
...
if __name__ == "__main__":
    ...
    # 保存知识图谱
    kg_builder.save_graph('knowledge_graph/knowledge_graph.pkl')
