...
class KnowledgeGraphBuilder:
    ...
    def create_user_to_user_relationships(self):
        """创建用户之间的直接关系（基于评论）"""
        self.logger.info("创建用户之间的直接关系（基于评论）...")

        # 直接从comments.json文件读取评论数据
        comments_file = "data/processed/integration/comments.json"
        self.logger.info(f"从 {comments_file} 读取评论数据...")
        ...
        for comment in tqdm(comments, desc="处理评论", unit="条"):
            ...
            mention_id = comment.get("mentionId", "")
            media_id = comment.get("postId", "")
            text = comment.get("text", "")

            # 只处理有mentionId的评论（直接提及了其他用户）
            if not mention_id or not user_id:
                continue
            ...
            if comment_id in self.comment_classifications:
                classification = self.comment_classifications[comment_id]
                offensive = classification["classification"]
            ...
            # 根据offensive分类添加到不同的关系列表
            if offensive == "offensive":
                offensive_comments.append({
                    "source": user_id,
                    "target": mention_id,
                    "properties": rel_props
                })
            else:
                non_offensive_comments.append({
                    "source": user_id,
                    "target": mention_id,
                    "properties": rel_props
                })
        ...
        # 创建关系
        with self.driver.session() as session:
            # 创建offensive_comment关系
            if offensive_comments:
                self.logger.info(f"创建 {len(offensive_comments)} 条offensive_comment关系...")
                session.run("""
                UNWIND $rels AS rel
                MATCH (source:User {id: rel.source})
                MATCH (target:User {id: rel.target})
                CREATE (source)-[r:OFFENSIVE_COMMENT]->(target)
                SET r.comment_id = rel.properties.comment_id,
                    r.media_id = rel.properties.media_id,
                    r.confidence = rel.properties.confidence,
                    r.text = rel.properties.text
                """, rels=offensive_comments)

            # 创建non_offensive_comment关系
            if non_offensive_comments:
                self.logger.info(f"创建 {len(non_offensive_comments)} 条non_offensive_comment关系...")
                session.run("""
                UNWIND $rels AS rel
                MATCH (source:User {id: rel.source})
                MATCH (target:User {id: rel.target})
                CREATE (source)-[r:NON_OFFENSIVE_COMMENT]->(target)
                SET r.comment_id = rel.properties.comment_id,
                    r.media_id = rel.properties.media_id,
                    r.confidence = rel.properties.confidence,
                    r.text = rel.properties.text
                """, rels=non_offensive_comments)

            self.logger.info(f"创建了 {len(offensive_comments)} 条offensive_comment关系和 {len(non_offensive_comments)} 条non_offensive_comment关系")

    def create_user_participates_relationships(self):
        """创建用户参与媒体会话的关系"""
        self.logger.info("创建用户参与媒体会话的关系...")
        ...
        for comment in tqdm(comments, desc="处理用户参与关系", unit="条"):
            user_id = comment.get("userId", "")
            media_id = comment.get("postId", "")
            comment_id = comment.get("commentId", "")
        ...
        with self.driver.session() as session:
            # 创建PARTICIPATES_IN关系
            if participates_rels:
                self.logger.info(f"创建 {len(participates_rels)} 条PARTICIPATES_IN关系...")
                session.run("""
                UNWIND $rels AS rel
                MATCH (u:User {id: rel.source})
                MATCH (m:MediaSession {id: rel.target})
                CREATE (u)-[r:PARTICIPATES_IN]->(m)
                SET r.comment_id = rel.properties.comment_id
                """, rels=participates_rels)

            self.logger.info(f"创建了 {len(participates_rels)} 条PARTICIPATES_IN关系")

    def create_comment_to_comment_relationships(self):
        """创建评论之间的直接关系（基于mention）"""
        self.logger.info("创建评论之间的直接关系（基于mention）...")
        ...
        # 收集关系数据
        offensive_mentions = []
        non_offensive_mentions = []

        # 处理评论
        for comment in tqdm(comments, desc="处理评论间关系", unit="条"):
            comment_id = comment.get("commentId", "")
            mention_id = comment.get("mentionId", "")
            media_id = comment.get("postId", "")
            text = comment.get("text", "")
            ...
            # 查找提及用户的所有评论
            mentioned_user_comments = []
            for other_comment in comments:
                if other_comment.get("userId", "") == mention_id and other_comment.get("postId", "") == media_id:
                    mentioned_user_comments.append(other_comment.get("commentId", ""))

            # 如果没有找到提及用户的评论，则跳过
            if not mentioned_user_comments:
                continue
            ...
            # 为每个提及用户的评论创建关系
            for target_comment_id in mentioned_user_comments:
                # 创建关系属性
                rel_props = {
                ...
                # 根据offensive分类添加到不同的关系列表
                if offensive == "offensive":
                    offensive_mentions.append({
                        "source": comment_id,
                        "target": target_comment_id,
                        "properties": rel_props
                    })
                else:
                    non_offensive_mentions.append({
                        "source": comment_id,
                        "target": target_comment_id,
                        "properties": rel_props
                    })
        ...
        # 创建关系
        with self.driver.session() as session:
            # 创建OFFENSIVE_MENTION关系
            if offensive_mentions:
                self.logger.info(f"创建 {len(offensive_mentions)} 条OFFENSIVE_MENTION关系...")
                session.run("""
                UNWIND $rels AS rel
                MATCH (source:Comment {id: rel.source})
                MATCH (target:Comment {id: rel.target})
                CREATE (source)-[r:OFFENSIVE_MENTION]->(target)
                SET r.source_comment_id = rel.properties.source_comment_id,
                    r.media_id = rel.properties.media_id,
                    r.confidence = rel.properties.confidence,
                    r.text = rel.properties.text
                """, rels=offensive_mentions)

            # 创建NON_OFFENSIVE_MENTION关系
            if non_offensive_mentions:
                self.logger.info(f"创建 {len(non_offensive_mentions)} 条NON_OFFENSIVE_MENTION关系...")
                session.run("""
                UNWIND $rels AS rel
                MATCH (source:Comment {id: rel.source})
                MATCH (target:Comment {id: rel.target})
                CREATE (source)-[r:NON_OFFENSIVE_MENTION]->(target)
                SET r.source_comment_id = rel.properties.source_comment_id,
                    r.media_id = rel.properties.media_id,
                    r.confidence = rel.properties.confidence,
                    r.text = rel.properties.text
                """, rels=non_offensive_mentions)

            self.logger.info(f"创建了 {len(offensive_mentions)} 条OFFENSIVE_MENTION关系和 {len(non_offensive_mentions)} 条NON_OFFENSIVE_MENTION关系")
    ...
    def build_knowledge_graph(self, data_file: str):
        ...
        try:
            ...
            # 创建增强的图结构
            self.logger.info("创建增强的图结构...")

            # 创建用户间直接边（基于评论）
            self.create_user_to_user_relationships()

            # 创建用户参与媒体会话的边
            self.create_user_participates_relationships()

            # 创建评论间直接边（基于mention）
            self.create_comment_to_comment_relationships()
