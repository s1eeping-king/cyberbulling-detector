...
class KnowledgeGraphBuilder:
    ...
    def normalize_by_field(self, items: List[Dict], field_names: List[str], method="minmax"):
        """
        Normalize numerical values for each field separately across all items using min-max normalization.

        Args:
            items: List of dictionaries (users, media_sessions, etc.)
            field_names: List of field names to normalize
            method: Normalization method (only "minmax" is supported now)

        This method adds normalized values as new fields with "_normalized" suffix
        """
        self.logger.info(f"Normalizing fields {field_names} across {len(items)} items using min-max normalization")
    ...
    def create_user_nodes(self, users: List[Dict]):
        ...
        # Normalize numerical values across all users using min-max normalization
        numerical_fields = ["followerCount", "followingCount", "likeCount", "postCount"]
        users = self.normalize_by_field(users, numerical_fields, method="minmax")
    ...
    def create_media_session_nodes(self, media_sessions: List[Dict]):
        ...
        # Normalize numerical values across all media sessions using min-max normalization
        numerical_fields = ["likeCount", "commentCount", "loopCount", "repostCount"]
        media_sessions = self.normalize_by_field(media_sessions, numerical_fields, method="minmax")
