...
class DataLoader:
    ...
    def _process_media_features(self, media) -> torch.Tensor:
        """处理媒体会话节点特征，整合RoBERTa特征"""
        if not media:
            # 返回一个占位符特征，维度为(1, 10+768)，包括原始特征和RoBERTa特征
        ...
        # 合并原始特征和RoBERTa特征
        features = torch.cat([original_features, roberta_features])
    ...
    def _process_comment_features(self, comments) -> Tuple[torch.Tensor, Dict[str, int]]:
        ...
        for comment in comments:
            ...
            if comment_id not in unique_comments:
                ...
                # 合并特征
                features = torch.cat([basic_features, roberta_features])
