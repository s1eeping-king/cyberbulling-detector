...
class DataLoader:
    ...
    def _process_media_features(self, media) -> torch.Tensor:
        """处理媒体会话节点特征，整合BERT特征"""
        if not media:
            # 返回一个占位符特征，维度为(1, 10+768)，包括原始特征和BERT特征
            return torch.zeros((1, 778), dtype=torch.float, device=self.device)
        ...
        if media_id in self.media_bert_features:
            ...
            # 如果没有找到BERT特征，使用零向量
            bert_features = torch.zeros(768, device=self.device)
    ...
    def _process_comment_features(self, comments) -> Tuple[torch.Tensor, Dict[str, int]]:
        ...
        if not comments:
            return torch.zeros((0, 770), dtype=torch.float, device=self.device), {}
