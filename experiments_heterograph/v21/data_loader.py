...
class DataLoader:
    ...
    def __init__(self, uri: str, user: str, password: str, device: torch.device, debug: bool = False):
        ...
        # 加载RoBERTa攻击性特征
        self.comment_roberta_offensive_features = self._load_roberta_offensive_features("data/processed/RoBERTa_features/comments_offensive.json", "comment")
        self.media_roberta_offensive_features = self._load_roberta_offensive_features("data/processed/RoBERTa_features/media_offensive.json", "media")
        ...
        print(f"已加载 {len(self.comment_roberta_offensive_features)} 条评论RoBERTa攻击性特征")
        print(f"已加载 {len(self.media_roberta_offensive_features)} 个媒体会话RoBERTa攻击性特征")
    ...
    def _load_roberta_offensive_features(self, file_path: str, feature_type: str) -> Dict[str, torch.Tensor]:
        """加载RoBERTa攻击性特征

        Args:
            file_path: RoBERTa攻击性特征文件路径
            feature_type: 特征类型 ('comment' 或 'media')
        ...
        """
        if not os.path.exists(file_path):
            print(f"警告: RoBERTa攻击性特征文件不存在: {file_path}")
            return {}
        ...
        if feature_type == "comment":
            ids = data.get("comment_ids", [])
            features = data.get("roberta_offensive_features", [])
        else:  # media
            ids = data.get("media_ids", [])
            features = data.get("roberta_offensive_features", [])
    ...
    def _process_media_features(self, media) -> torch.Tensor:
        """处理媒体会话节点特征，整合RoBERTa特征和RoBERTa攻击性特征"""
        if not media:
            # 返回一个占位符特征，维度为(1, 10+768+768)，包括原始特征、RoBERTa特征和RoBERTa攻击性特征
            return torch.zeros((1, 1546), dtype=torch.float, device=self.device)
        ...
        # 获取RoBERTa攻击性特征
        if media_id in self.media_roberta_offensive_features:
            roberta_offensive_features = self.media_roberta_offensive_features[media_id].to(self.device)
        else:
            # 如果没有找到RoBERTa攻击性特征，使用零向量
            roberta_offensive_features = torch.zeros(768, device=self.device)

        # 合并原始特征、RoBERTa特征和RoBERTa攻击性特征
        features = torch.cat([original_features, roberta_features, roberta_offensive_features])
    ...
    def _process_comment_features(self, comments) -> Tuple[torch.Tensor, Dict[str, int]]:
        ...
        if not comments:
            return torch.zeros((0, 1538), dtype=torch.float, device=self.device), {}
        ...
        for comment in comments:
            ...
            if comment_id not in unique_comments:
                ...
                # RoBERTa攻击性特征
                if comment_db_id in self.comment_roberta_offensive_features:
                    roberta_offensive_features = self.comment_roberta_offensive_features[comment_db_id].to(self.device)
                else:
                    roberta_offensive_features = torch.zeros(768, device=self.device)

                # 合并特征：基本特征 + RoBERTa特征 + RoBERTa攻击性特征
                features = torch.cat([basic_features, roberta_features, roberta_offensive_features])
