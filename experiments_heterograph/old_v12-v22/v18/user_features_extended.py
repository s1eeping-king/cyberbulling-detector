...
class UserFeaturesExtractor:
    ...
    def extract_user_features(self, user_ids: List[str], statistic_media_sessions: List[str] = None) -> List[Dict[str, Any]]:
        ...
        # 如果提供了statistic_media_sessions，记录日志
        if statistic_media_sessions is not None:
            self.logger.info(f"使用 {len(statistic_media_sessions)} 个媒体会话计算统计特征")
        ...
        for user_id in tqdm(user_ids, desc="提取用户特征"):
            ...
            # 获取mention相关特征（基于媒体会话）
            mention_features = self.get_user_mention_features(user_id, statistic_media_sessions)
            user_features.update(mention_features)

            # 获取评论相关特征（基于媒体会话）
            comment_features = self.get_user_comment_features(user_id, statistic_media_sessions)
    ...
    def get_user_mention_features(self, user_id: str, statistic_media_sessions: List[str] = None) -> Dict[str, Any]:
        """获取用户mention相关特征

        Args:
            user_id: 用户ID
            statistic_media_sessions: 用于计算统计特征的媒体会话ID列表，如果为None，则不限制媒体会话范围
        ...
        # 如果提供了statistic_media_sessions，则限制查询范围到指定的媒体会话集
        if statistic_media_sessions is not None:
            # 1. mention次数 (用户发出的总mention数量) - 基于媒体会话集
            query1 = """
            MATCH (u:User {id: $userId})-[:CREATES]->(c:Comment)-[:BELONGS_TO]->(m:MediaSession), (c)-[:MENTIONS]->(target:User)
            ...
            # 2. 被mention次数 (用户收到的总mention数量) - 基于媒体会话集
            query2 = """
            MATCH (c:Comment)-[:BELONGS_TO]->(m:MediaSession), (c)-[:MENTIONS]->(u:User {id: $userId})
            WHERE m.id IN $media_ids
            WITH
                count(c) AS total_mentions,
                sum(CASE WHEN c.offensive = 'offensive' THEN 1 ELSE 0 END) AS offensive_count,
            ...
            query7 = """
            // 基于媒体会话集计算双向OFFENSIVE关系 - 优化版本
            MATCH (u:User {id: $userId})-[:CREATES]->(c1:Comment)-[:BELONGS_TO]->(m1:MediaSession), (c1)-[:MENTIONS]->(other:User)
            WHERE m1.id IN $media_ids AND c1.offensive = 'offensive'
            WITH DISTINCT other
            WHERE EXISTS {
                MATCH (c2:Comment)-[:BELONGS_TO]->(m2:MediaSession), (c2)-[:MENTIONS]->(u:User {id: $userId})
                WHERE m2.id IN $media_ids AND c2.offensive = 'offensive'
            ...
            query8 = """
            // 基于媒体会话集计算双向NON_OFFENSIVE关系 - 优化版本
            MATCH (u:User {id: $userId})-[:CREATES]->(c1:Comment)-[:BELONGS_TO]->(m1:MediaSession), (c1)-[:MENTIONS]->(other:User)
            WHERE m1.id IN $media_ids AND (c1.offensive = 'not_offensive' OR c1.offensive IS NULL)
            WITH DISTINCT other
            WHERE EXISTS {
                MATCH (c2:Comment)-[:BELONGS_TO]->(m2:MediaSession), (c2)-[:MENTIONS]->(u:User {id: $userId})
                WHERE m2.id IN $media_ids AND (c2.offensive = 'not_offensive' OR c2.offensive IS NULL)
                AND EXISTS((other)-[:CREATES]->(c2))
            }
            ...
            query9 = """
            // 基于媒体会话集计算未反击用户数 - 优化版本
            MATCH (c:Comment)-[:BELONGS_TO]->(m:MediaSession), (c)-[:MENTIONS]->(u:User {id: $userId}), (source:User)-[:CREATES]->(c)
            WHERE m.id IN $media_ids AND c.offensive = 'offensive'
            WITH DISTINCT source
            WHERE NOT EXISTS {
                MATCH (u:User {id: $userId})-[:CREATES]->(c2:Comment)-[:BELONGS_TO]->(m2:MediaSession), (c2)-[:MENTIONS]->(source)
                WHERE m2.id IN $media_ids AND c2.offensive = 'offensive'
            }
        ...
        with self.driver.session() as session:
            # 根据是否有statistic_media_sessions来决定查询参数
            if statistic_media_sessions is not None:
                params = {"userId": user_id, "media_ids": statistic_media_sessions}
                ...
                incoming_offensive_query = """
                    // 基于媒体会话集计算向用户发送过OFFENSIVE评论的用户数 - 优化版本
                    MATCH (c:Comment)-[:BELONGS_TO]->(m:MediaSession), (c)-[:MENTIONS]->(u:User {id: $userId}), (source:User)-[:CREATES]->(c)
                    WHERE m.id IN $media_ids AND c.offensive = 'offensive'
    ...
    def get_user_comment_features(self, user_id: str, statistic_media_sessions: List[str] = None) -> Dict[str, Any]:
        """获取用户评论相关特征
        ...
        """
        # 根据是否有statistic_media_sessions来决定查询
        if statistic_media_sessions is not None:
            # 限制查询范围的版本 - 基于媒体会话集
            # 1. 用户的Comment数量
            query1 = """
            ...
            query3 = """
            MATCH (u:User {id: $userId})-[:CREATES]->(c:Comment)-[:BELONGS_TO]->(m:MediaSession)
            WHERE m.id IN $media_ids AND (c.offensive = 'not_offensive' OR c.offensive IS NULL)
            ...
            query8 = """
            MATCH (u:User {id: $userId})-[:CREATES]->(c:Comment)-[:BELONGS_TO]->(m:MediaSession)
            WHERE m.id IN $media_ids AND c.offensive = 'offensive' AND c.created IS NOT NULL
