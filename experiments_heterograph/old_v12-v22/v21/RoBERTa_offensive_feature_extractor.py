...
class RoBERTaOffensiveFeatureExtractor:
    """RoBERTa攻击性特征提取器，用于从文本中提取攻击性语义特征"""

    def __init__(self, model_name='cardiffnlp/twitter-roberta-base-offensive', device=None, output_dim=768):
        """
        初始化RoBERTa攻击性特征提取器
        ...
        print(f"RoBERTa攻击性特征提取器使用设备: {self.device}")
    ...
    def extract_features(self, texts, batch_size=16, max_length=128):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="提取RoBERTa攻击性特征"):
                # 将输入移动到设备
    ...
    def process_comments(self, comments_file, output_file, batch_size=16):
        ...
        result = {
            ...
            'roberta_offensive_features': features_np.tolist()
        }
    ...
    def process_media_sessions(self, media_file, output_file, batch_size=16):
        ...
        print(f"媒体会话RoBERTa攻击性特征已保存至: {output_file}")
        print(f"特征维度: {features.shape}")
...
def main():
    ...
    # 初始化RoBERTa攻击性特征提取器 - 使用原始768维RoBERTa特征
    extractor = RoBERTaOffensiveFeatureExtractor(output_dim=768, device=device)

    # 处理评论数据
    comments_file = "data/processed/integration/comments.json"
    comments_output = os.path.join(output_dir, "comments_offensive.json")

    if not os.path.exists(comments_output):
        print("开始处理评论数据...")
        extractor.process_comments(comments_file, comments_output)
    else:
        print(f"评论RoBERTa攻击性特征文件已存在: {comments_output}")

    # 处理媒体会话数据
    media_file = "data/processed/integration/media_sessions.json"
    media_output = os.path.join(output_dir, "media_offensive.json")

    if not os.path.exists(media_output):
        print("开始处理媒体会话数据...")
        extractor.process_media_sessions(media_file, media_output)
    else:
        print(f"媒体会话RoBERTa攻击性特征文件已存在: {media_output}")

    print("RoBERTa攻击性特征提取完成!")
