...
def create_oversampled_training_data(data_loader: DataLoader, train_ids: List[str]):
    """创建过采样的训练数据
    ...
    print(f"原始训练集: 霸凌样本 {len(pos_ids)} 个, 非霸凌样本 {len(neg_ids)} 个")
    print(f"原始比例: 1:{len(neg_ids)/len(pos_ids):.2f}")

    # 对霸凌样本进行2倍过采样
    oversampled_pos_ids = pos_ids  # 简单复制一倍
    ...
    print(f"过采样后训练集: 霸凌样本 {len(oversampled_pos_ids)} 个, 非霸凌样本 {len(neg_ids)} 个")
    print(f"过采样后比例: 1:{len(neg_ids)/len(oversampled_pos_ids):.2f}")
    print(f"总训练样本数: {len(oversampled_ids)}")
...
def train_model(model: SimpleGAT,
    ...
    # 创建过采样的训练数据
    oversampled_train_ids = create_oversampled_training_data(data_loader, train_ids)
...
def evaluate_model(model: SimpleGAT, data_loader: DataLoader, test_ids: List[str], batch_size: int = 64):
    """评估模型在测试集上的性能"""
    device = next(model.parameters()).device
    model.eval()
...
def main():
    ...
    # 创建GAT模型 - 增加隐藏层维度以适应更大的RoBERTa特征
    model = SimpleGAT(
