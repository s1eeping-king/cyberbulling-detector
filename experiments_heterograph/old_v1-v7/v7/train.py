...
from HAN import SimpleHAN, calculate_metrics

def train_model(model: SimpleHAN,
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(train_ids), batch_size):
            ...
            try:
                ...
                edge_index_dict = {k: v.to(device) for k, v in batch_data['edge_index_dict'].items() if isinstance(k, tuple) and len(k) == 3}
                # 保存批次索引，但不添加到edge_index_dict
                batch_dict = None
                if 'batch_dict' in batch_data:
                    batch_dict = {k: v.to(device) for k, v in batch_data['batch_dict'].items()}
                # 处理边属性 - 只保留符合元路径格式的键
                edge_attr_dict = None
