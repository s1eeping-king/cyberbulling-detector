...
class DataLoader:
    ...
    def load_subgraph(self, media_session_id: str) -> HeteroData:
        ...
        // 原始关系
        OPTIONAL MATCH (publisher:User)-[:PUBLISHES]->(m)
        OPTIONAL MATCH (u:User)-[:CREATES]->(c:Comment)-[:BELONGS_TO]->(m)
        OPTIONAL MATCH (c)-[:MENTIONS]->(mentioned:User)

        // 用户间直接边
        OPTIONAL MATCH (commenter:User)-[r3:OFFENSIVE_COMMENT]->(target:User)
        WHERE r3.media_id = $media_id
        OPTIONAL MATCH (commenter2:User)-[r4:NON_OFFENSIVE_COMMENT]->(target2:User)
        WHERE r4.media_id = $media_id
        ...
        RETURN m,
               ...
               collect(distinct commenter) as offensive_commenters,
               collect(distinct target) as offensive_targets,
               collect(distinct commenter2) as non_offensive_commenters,
               collect(distinct target2) as non_offensive_targets,
               ...
               // 用户间直接边
               collect(distinct [elementId(commenter), elementId(target), r3.comment_id]) as offensive_comment_rels,
               collect(distinct [elementId(commenter2), elementId(target2), r4.comment_id]) as non_offensive_comment_rels
        """
        ...
        with self.driver.session() as session:
            ...
            all_users = (result['publishers'] + result['users'] + result['mentioned_users'] +
                        result['offensive_commenters'] + result['offensive_targets'] +
                        result['non_offensive_commenters'] + result['non_offensive_targets'])
            ...
            # 3. 处理评论节点特征
            comment_features, comment_id_to_idx = self._process_comment_features(result['comments'])
            ...
            non_offensive_edge_index = self._process_direct_comment_edge(
                result['non_offensive_comment_rels'],
                {'user': data['user'].x.shape[0]},
                user_id_to_idx
            )
