...
class BertFeatureExtractor:
    ...
    def __init__(self, model_name='bert-base-uncased', device=None, output_dim=768):
        """
        初始化BERT特征提取器
        ...
        Args:
            ...
            output_dim: 输出特征维度 (默认为768，即BERT原始维度)
        """
        if device is None:
        ...
        # 如果输出维度不是768，则创建降维层
        if self.output_dim != self.bert_dim:
    ...
    def extract_features(self, texts, batch_size=16, max_length=128):
        ...
        Returns:
            features: 特征张量 [n_texts, output_dim]
                     ...
                     如果use_dim_reducer=False，则为原始BERT特征 (768维)
        """
        # 创建数据集和数据加载器
...
def main():
    ...
    # 初始化BERT特征提取器 - 使用原始768维BERT特征
    extractor = BertFeatureExtractor(output_dim=768, device=device)
